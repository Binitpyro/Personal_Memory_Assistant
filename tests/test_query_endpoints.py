...
@pytest.mark.asyncio
async def test_api_query_standard(client: AsyncClient, mock_db, mock_emb, mock_lancedb, override_llm):
...
@pytest.mark.asyncio
async def test_api_query_stream_standard(client: AsyncClient, mock_db, mock_emb, mock_lancedb, override_llm):
    mock_lancedb.search_cache = AsyncMock(return_value=None)
    mock_lancedb.add_query_cache = AsyncMock()
    ...
    app.dependency_overrides[get_llm] = lambda: override_llm
    try:
        response = await client.post("/api/query/stream", json={
...
@pytest.mark.asyncio
async def test_api_query_fast_path_metadata(client: AsyncClient, mock_db, mock_emb, mock_lancedb, override_llm):
    mock_lancedb.search_cache = AsyncMock(return_value=None)
...
@pytest.mark.asyncio
async def test_api_query_fast_path_project(client: AsyncClient, mock_db, mock_emb, mock_lancedb, override_llm):
    mock_lancedb.search_cache = AsyncMock(return_value=None)
...
@pytest.mark.asyncio
async def test_query_challenge_mode(client: AsyncClient, mock_db, mock_emb, mock_lancedb, override_llm):
    mock_lancedb.search_cache = AsyncMock(return_value=None)
    mock_lancedb.semantic_search = AsyncMock(return_value={
        "ids": [["1"]],
        "distances": [[0.1]],
    ...
    try:
        response = await client.post("/api/query", json={
            "question": "Explain implementation details",
            "mode": "challenge"
        })
        assert response.status_code == 200
        data = response.json()
...
async def test_retrieval_semantic_cache_hit(mock_db, mock_emb, mock_lancedb, override_llm):
    mock_lancedb.search_cache = AsyncMock(return_value={
