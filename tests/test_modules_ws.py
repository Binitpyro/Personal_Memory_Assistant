...
def test_websocket_connection_unauthorized():
    ...
    # 1. No token provided
    with pytest.raises(Exception):  # TestClient raises WebSocketDisconnect or RuntimeError
...
def test_websocket_connection_authorized():
    ...
    with client.websocket_connect("/api/modules/ws", headers={"X-Local-Access-Token": token}) as websocket:
        websocket.send_json({"action": "ping"})
        response = websocket.receive_json()
        assert response == {"status": "pong"}
