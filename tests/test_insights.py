...
async def test_insights_endpoints(client, mock_db):
    ...
    mock_service.get_insights_for_extension = AsyncMock(return_value={"extension": ".py", "count": 10})
...
async def test_visualizer_binary_stream_endpoint(client, mock_db):
    ...
    assert len(content) == 64 # 2 records * 32 bytes
    
    # Unpack first record: pos(fff) radius(f) parent_index(I) flags(I) type_hash(I) pad(I)
    x, y, z, radius, parent_index, flags, type_hash, pad = struct.unpack("<ffff I I I I", content[:32])
    assert radius > 2.0
    ...
    assert len(content_filtered) == 32 # Only 1 file matching .py

    # 3. Exception path
    with patch("app.insights.visualizer._stream_visualizer_binary_impl", side_effect=Exception("Stream generation failed")):
