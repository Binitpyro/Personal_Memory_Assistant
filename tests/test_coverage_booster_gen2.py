...
from app.scanner.scanner import scan_folder, ScanResult, _scandir_walk, _list_dir_entries
...
from app.search.retrieval import _get_metadata_insights, _append_latest_files, _append_largest_files
from app.storage.db import DatabaseManager, serialize_write
from app.indexing.summarizer import generate_deep_summary, _summarize_python, _summarize_code_regex, _summarize_markdown, _summarize_data_format, _summarize_spreadsheet_text, _summarize_doc_text
from app.indexing.service import _get_sentence_offsets, IndexingService
...
def test_get_app_version_importlib_fails(monkeypatch):
    ...
    monkeypatch.setattr("importlib.metadata.version", lambda name: exec('raise Exception("failed")'))
...
def test_ntfs_scanner_mocked_execution(monkeypatch, tmp_path):
    ...
    calls = 0
    def mock_device_io_control(handle, ioctl, in_buf, in_len, out_buf, out_len, bytes_returned, overlapped):
        nonlocal calls
        if calls == 0:
            calls += 1
            fake_records = [
...
def test_context_builder_edge_cases():
    ...
    res, count = build_context([], max_tokens=100)
    assert res == "No relevant context found."

    # 2. default context max tokens when <= 0
    from app.config import settings
    settings.context_max_tokens = 2500
    res, count = build_context([{"file_path": "a.py", "text": "foo", "score": 1.0}], max_tokens=0)
    assert "a.py" in res
...
def test_context_builder_tiktoken_error(monkeypatch):
    ...
    if "tiktoken" in sys.modules:
        monkeypatch.setattr("tiktoken.get_encoding", lambda name: exec('raise Exception("failed")'))
...
def test_semantic_deduplicate_with_datasketch(monkeypatch):
    ...
    mock_lsh = MagicMock()
    mock_lsh.query.side_effect = [[], ["res_0"]]  # first call empty, second call duplicate match
    
    mock_minhash = MagicMock()
    
    with patch.dict("sys.modules", {"datasketch": MagicMock(), "datasketch.MinHash": mock_minhash, "datasketch.MinHashLSH": mock_lsh}):
...
async def test_get_metadata_insights_latest_and_largest():
    ...
    db = AsyncMock()
    db.execute_query = AsyncMock(side_effect=lambda sql, *args: [
        ("C:/path/to/latest.py", "2026-07-04")
    ] if "modified_at" in sql else [
        ("C:/path/to/largest.py", 10 * 1024 * 1024)
    ])
...
async def test_db_manager_frozen_path(monkeypatch, tmp_path):
    ...
    fake_schema.write_text("CREATE TABLE files (id INTEGER); CREATE TABLE chunks (id INTEGER);", encoding="utf-8")
...
async def test_db_manager_savepoint_transaction_handling(tmp_path):
    ...
    with patch("os.path.exists", return_value=False):
        ...
        await db.execute_write("INSERT INTO files (path, size, modified_at) VALUES (?, ?, ?)", ("C:/stale.py", 100, "now"))
...
def test_get_sentence_offsets_nltk_exception(monkeypatch):
    ...
    monkeypatch.setattr("nltk.tokenize.sent_tokenize", lambda text: exec('raise Exception("failed")'), raising=False)
