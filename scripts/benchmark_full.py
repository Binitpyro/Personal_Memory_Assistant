    ...
    def _monitor(self):
        ...
        while self.running:
            try:
                ...
                pass
            print(f"[MemoryMonitor] Current RSS: {rss / 1024 / 1024:.2f} MB | Peak: {self.peak_rss / 1024 / 1024:.2f} MB", flush=True)
            time.sleep(10.0)
...
async def run_benchmark():
    ...
    # 1. Cold Run
    cold_results = await run_indexing_pass('Cold', indexing_service, monitor)
    ...
    # 3. Incremental Run
    incremental_results = await run_indexing_pass('Incremental', indexing_service, monitor)
