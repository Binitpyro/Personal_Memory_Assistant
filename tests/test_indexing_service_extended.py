...
class FakeDB:
    ...
    async def get_file_by_path(self, path):
        if path in self.files: return {"id": self.files[path]}
        return None
    ...
    async def batch_insert_files(self, files_data):
        ids = []
        for fd in files_data: ids.append(await self.insert_file(fd))
        return ids
    ...
    async def insert_chunk_embeddings_bulk(self, rows): return None
    async def commit(self): return None
    async def upsert_folder_profile(self, profile, *, auto_commit=True): self.profile_rows.append(profile)
    async def get_existing_file_ids(self, paths): return {p: fid for p, fid in self.files.items() if p in paths}
    async def execute_write(self, q, p): return None
    async def wal_checkpoint(self): return None
...
def test_project_detection_and_overlap_resolution(tmp_path: Path):
    ...
    src = project / "src"; src.mkdir(parents=True)
    pkg = project / "package.json"; pkg.write_text("{}", encoding="utf-8")
    f = src / "main.js"; f.write_text("console.log('x')", encoding="utf-8")
...
def test_folder_profile_and_chunk_helpers(tmp_path: Path):
    ...
    py = folder / "a.py"; py.write_text("print('a')", encoding="utf-8")
    md = folder / "README.md"; md.write_text("hello", encoding="utf-8")
...
async def test_stream_extract_and_prepare(tmp_path: Path):
    ...
    header = await q.get(); assert header["type"] == "header"
    chunk = await q.get(); assert chunk["type"] == "chunk"
    footer = await q.get(); assert footer["type"] == "footer"
...
async def test_scan_index_file_and_profiles(monkeypatch, tmp_path: Path):
    ...
    file1 = folder / "one.txt"; file1.write_text("A short document.", encoding="utf-8")
...
def test_extract_monolithic_and_chunking(tmp_path: Path):
    svc = _make_service()
    strict_json = tmp_path / "a.json"; strict_json.write_text('{"x": 1}', encoding="utf-8")
