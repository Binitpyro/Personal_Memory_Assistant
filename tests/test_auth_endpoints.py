...
async def test_get_client_config(tmp_path):
    ...
    with patch("app.api.auth.CLIENT_SECRETS_FILE", tmp_path / "missing.json"):
        with patch.dict(
            os.environ, {"GOOGLE_CLIENT_ID": "env_id", "GOOGLE_CLIENT_SECRET": "env_secret"}
        ):
            config = await get_client_config()
            assert config["installed"]["client_id"] == "env_id"
            assert config["installed"]["client_secret"] == "env_secret"  # noqa: S105
...
async def test_start_oauth_success(client):
    ...
    with patch("app.api.auth.get_client_config", return_value={"installed": {}}):  # noqa: SIM117
...
async def test_callback_success(client, patch_token_path):
    ...
    mock_creds.token = "token123"  # noqa: S105
    mock_creds.refresh_token = "refresh123"  # noqa: S105
    mock_creds.token_uri = "uri123"  # noqa: S105
    mock_creds.client_id = "cid123"
    mock_creds.client_secret = "csec123"  # noqa: S105
    ...
    with patch("app.api.auth.get_client_config", return_value={"installed": {}}):  # noqa: SIM117
        with patch("app.api.auth.Flow.from_client_config", return_value=mock_flow_instance):
            ...
            assert data["token"] == "token123"  # noqa: S105
...
async def test_status_valid_token(client, patch_token_path):
    ...
    with patch(
        "app.api.auth.Credentials.from_authorized_user_info", return_value=mock_creds_instance
    ):
        response = await client.get("/api/auth/google/status")
        assert response.status_code == 200
...
async def test_status_expired_refreshed(client, patch_token_path):
    ...
    mock_creds_instance.refresh_token = "r1"  # noqa: S105
    mock_creds_instance.token = "t2"  # noqa: S105

    with (
        patch(
            "app.api.auth.Credentials.from_authorized_user_info", return_value=mock_creds_instance
        ),
        patch.object(mock_creds_instance, "refresh") as mock_refresh,
    ):
        response = await client.get("/api/auth/google/status")
        assert response.status_code == 200
        assert response.json() == {"connected": True, "method": "oauth"}
        ...
        assert saved["token"] == "t2"  # noqa: S105
...
async def test_status_exception(client, patch_token_path):
    patch_token_path.write_text(json.dumps({"token": "t1"}))
    with patch(
        "app.api.auth.Credentials.from_authorized_user_info", side_effect=Exception("parse error")
    ):
        response = await client.get("/api/auth/google/status")
        assert response.status_code == 200
        assert response.json() == {"connected": False}
...
async def test_keyring_endpoints(client):
    ...
    with patch("app.api.auth.keyring.set_password") as mock_set:
        response = await client.post(
            "/api/auth/google/keys", json={"provider": "gemini", "api_key": "my_long_key_value"}
        )
        assert response.status_code == 200
    ...
    with patch("app.api.auth.keyring.set_password", side_effect=Exception("Keyring error")):
        response = await client.post(
            "/api/auth/google/keys", json={"provider": "gemini", "api_key": "my_key"}
        )
        assert response.status_code == 500
