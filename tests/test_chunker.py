...
import rust_core
from app.indexing.service import StreamChunker, _get_sentence_offsets, IndexingService
...
def test_rust_cjk_offsets():
    ...
    """
    text = "你好，世界！这是一个测试。我们需要确保中文字符的偏移量在 Rust 和 Python 之间是完全一致的。"
    chunk_size = 10
    chunk_overlap = 2
    prefix = "[test] "
    ...
    for chunk in chunks:
        ...
        # Verify that Python character slice text[start:end] matches the content in text_preview (excluding prefix)
...
def test_create_chunks_routing(monkeypatch):
    ...
    called.clear()
    monkeypatch.setattr(idx_service.code_chunker, "chunk_code", lambda *args, **kwargs: [{"text_preview": "py_chunk"}])
