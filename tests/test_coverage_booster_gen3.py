...
async def test_db_manager_coverage_booster(mock_db):
    ...
    assert await db.get_existing_file_ids(["C:/file1.py", "C:/nonexistent.py"]) == {"C:/file1.py": file_id}
    assert await db.get_files_modified_map(["C:/file1.py"]) == {"C:/file1.py": "2026-07-05T20:00:00Z"}
    assert await db.get_files_sha256_map(["C:/file1.py"]) == {"C:/file1.py": "hash123"}
    assert await db.get_files_change_map(["C:/file1.py"]) == {"C:/file1.py": ("2026-07-05T20:00:00Z", "hash123")}
    ...
    assert len(chunks) == 1
    assert chunks[0]["text_preview"] == b"def my_chunk(): pass"
    ...
    await db.execute_write(
        "INSERT INTO kg_nodes (id, label, properties) VALUES (?, ?, ?)",
        ("node1", "Class", '{"chunk_id": 10}')
    )
    await db.execute_write(
        "INSERT INTO kg_nodes (id, label, properties) VALUES (?, ?, ?)",
        ("node2", "Method", '{"chunk_id": 11}')
    )
    await db.execute_write(
    ...
    await db.execute_write(
        "INSERT INTO folder_profiles (folder_path, project_type, file_count, total_size_bytes) VALUES (?, ?, ?, ?)",
        ("C:/FolderA", "Python", 1, 100)
    )
    nodes = []
...
def test_indexing_service_extract_plain_text_stream(tmp_path):
    service = IndexingService(db=MagicMock(), embedding_service=MagicMock(), lancedb_client=MagicMock())
...
async def test_scan_all_folders_rust_mocked(monkeypatch):
    ...
    files, method, dur = service._scan_all_folders([Path("C:/FolderA")])
    assert len(files) == 1
    ...
    files_fallback, method_fallback, _ = service._scan_all_folders([Path("C:/FolderA")])
