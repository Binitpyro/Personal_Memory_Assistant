...
LATEST_RE = re.compile(r'\b(?:latest|recent|newest|added lately|last updated|last modified)\b', re.IGNORECASE)
LARGEST_RE = re.compile(r'\b(?:largest|biggest|huge|oversized|most space|taking up space)\b', re.IGNORECASE)
...
def determine_query_intent(query: str) -> Dict[str, bool]:
    q = query.lower()
    return {
        "inventory": bool(LATEST_RE.search(query) or LARGEST_RE.search(query) or "how many files" in q),
...
PROJECT_SIGNATURES = [
    ...
    ("Python", PYTHON_PROJECT_LABEL, [("file", "pyproject.toml")]),
