...
class MockEmbeddingService:
    def __init__(self):
        ...
        Sets the service as ready and creates an empty in-memory cache for query embeddings.
        """
        self._is_ready = True
        self._query_cache = {}
    @property
    def is_ready(self): """
Indicates whether the embedding service is ready.
...
Produce a deterministic 384-dimensional embedding for the given text (mock implementation).

Returns:
    A list of 384 floats where each element is 0.1.
"""
return [0.1] * 384
    async def embed_texts(self, texts, batch_size=None):
        """
        Return fixed 384-dimensional embedding vectors for each input text.
        
        Parameters:
            texts (Iterable[str]): Sequence of input texts to embed.
            batch_size (int | None): Optional batch size hint; ignored by this mock.
        
        Returns:
            list[list[float]]: A list where each element is a 384-length vector of 0.1 corresponding to each input text.
        """
        return [[0.1] * 384 for _ in texts]
    def load_model_background(self): """
No-op placeholder for initiating background model loading.

This implementation performs no action and exists to satisfy the embedding service interface in tests and mocks.
"""
pass
...
class MockChromaClient:
    async def semantic_search(self, embedding, k, where_filter=None):
        """
        Return a deterministic mock semantic search result used for testing.
        
        Parameters:
            embedding: The query embedding (unused by this mock).
            k: Number of nearest neighbors requested (unused by this mock).
            where_filter: Optional filter applied to the search (unused by this mock).
        
        Returns:
            dict: A fixed result with keys:
                - `ids`: list of lists of string IDs (here `[["1"]]`).
                - `distances`: list of lists of distances (here `[[0.1]]`).
                - `metadatas`: list of lists of metadata dicts (each containing `file_path` and `text`).
        """
        return {"ids": [["1"]], "distances": [[0.1]], "metadatas": [[{"file_path": "a.py", "text": "hello"}]]}
    async def search_summaries(self, embedding, k):
        """
        Return mock summary search results for the provided embedding.
        
        Parameters:
            embedding (Sequence[float]): Embedding vector used to perform the semantic search.
            k (int): Number of nearest summary results requested.
        
        Returns:
            dict: A search-result mapping with keys:
                - `ids`: list of lists of result IDs (one list per query).
                - `distances`: list of lists of distances corresponding to each ID.
                - `metadatas`: list of lists of metadata dicts for each result, each containing
                  `file_path` and `folder_tag`.
        """
        return {"ids": [["s1"]], "distances": [[0.1]], "metadatas": [[{"file_path": "a.py", "folder_tag": "t"}]]}
    async def add_documents(self, ids, embeddings, metadatas): """
Accepts document insertion payloads but performs no action (test stub).

Parameters:
    ids (Sequence[str|int]): Sequence of document identifiers corresponding to the embeddings.
    embeddings (Sequence[Sequence[float]]): Sequence of embedding vectors, one per id.
    metadatas (Sequence[dict]): Sequence of metadata dictionaries, one per id.
"""
pass
    async def add_summaries_batch(self, items): """
Accept a batch of summary items and perform no operation (mock implementation used for testing).

Parameters:
    items (Iterable[dict|object]): Batch of summary records to add; structure is not validated by this mock.
"""
pass
    async def delete_documents(self, ids): """
Delete the documents identified by `ids` from the store (no-op in this mock).

Parameters:
    ids (Iterable[str] | Sequence[str]): Identifiers of the documents to delete.
"""
pass
    async def delete_folder_data(self, tag): """
Delete all vector-store entries associated with the given folder tag; no operation in this mock implementation.

Parameters:
    tag (str): Folder tag identifying which documents/summaries to remove.
"""
pass
    def connect(self): """
No-op connect method to satisfy the Chroma client interface.

This method performs no action and is provided so tests can call `connect()` on the mock client without side effects.
"""
pass
...
async def real_db():
    """
    Provide a DatabaseManager backed by a per-process temporary SQLite file for tests.
    
    Initializes the database schema, yields the connected DatabaseManager for test use, and after the test closes the connection and attempts to delete the on-disk database file (ignoring deletion errors).
    
    Yields:
        DatabaseManager: A manager connected to a temporary SQLite file named `test_pma_{pid}.db`.
    """
...
def client(real_db):
    """
    Provide a TestClient configured to use the provided test database.
    
    Parameters:
        real_db (DatabaseManager): The test database instance to inject into the application's
            dependency that returns a database.
    
    Returns:
        TestClient: A TestClient whose `get_db` dependency is overridden to return `real_db`.
        The override is cleared after the client context exits.
    """
