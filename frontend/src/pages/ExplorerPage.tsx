...
/**
 * Renders a collapsible folder node with its nested subfolders and files, providing selection and deletion controls.
 *
 * The folder header is keyboard-accessible and toggles expansion with Enter or Space. File rows are keyboard-accessible and invoke `onSelect` when activated. Deleting the folder prompts for confirmation and calls `onDeleteFolder` with the folder's `fullPath` when confirmed.
 *
 * @param node - The tree node to render (name, fullPath, children, and files)
 * @param depth - The depth of this node in the tree; root nodes typically have depth 0
 * @param onSelect - Callback invoked with a `FileEntry` when a file row is selected
 * @param selectedPath - The currently selected file path used to apply selected styling
 * @param onDeleteFolder - Callback invoked with the folder `fullPath` after user confirmation to delete the folder index
 * @returns A JSX element representing the folder node and its expandable contents
 */
...
/**
 * Render the Explorer page UI for browsing indexed files and folders, selecting items, applying an extension filter, and deleting folder indexes; supports tree and treemap views.
 *
 * @returns The Explorer page React element.
 */
