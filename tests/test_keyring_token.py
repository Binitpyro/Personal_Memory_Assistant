...
def test_token_already_in_environment():
    ...
    with patch.dict(os.environ, {"X_LOCAL_ACCESS_TOKEN": "pre-existing-token"}), \
         patch("keyring.get_password") as mock_get:
        
        _init_local_access_token()
        
        assert os.environ["X_LOCAL_ACCESS_TOKEN"] == "pre-existing-token"
...
def test_token_retrieved_from_keyring():
    ...
    with patch.dict(os.environ, {}), \
        ...
        assert os.environ["X_LOCAL_ACCESS_TOKEN"] == "token-from-keyring"
...
def test_keyring_exception_fallback():
    ...
    with patch.dict(os.environ, {}), \
         patch("keyring.get_password", side_effect=Exception("Keyring is broken")) as mock_get, \
