...
* **The local access token is header-only** (`d56b94e`, S-1). The middleware accepted `?token=` on every `/api/` path as a fallback; query strings reach uvicorn's access log, browser history and `Referer`. It had no producers — the frontend sends the header on every request and only *reads* `?token=` off the page URL.
  * **Corrected 2026-08-21:** "header-only" was overstated when first written here. `/api/modules/ws` does not traverse that middleware and kept its own `?token=` fallback, so the policy had a hole in it for the rest of this cycle. Closed after a PR review flagged it; the socket is header-only too now. The usual objection — browsers cannot set headers on a WebSocket handshake — did not apply, because nothing in `frontend/src` opens a WebSocket at all.
