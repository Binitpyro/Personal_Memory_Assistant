...
def patch_data_paths(tmp_path):
    ...
    with (
        patch("app.search.llm_client.Path", lambda *args: {
...
async def test_llm_client_ensure_token_loaded(patch_data_paths):
    ...
    assert client._oauth_token == "mock_oauth"
...
async def test_refresh_token_if_expired(tmp_path):
    ...
    with patch("google.auth.transport.requests.Request", MagicMock()):
        ...
        assert token_data["token"] == "refreshed_token"
        assert token_file.exists()
        saved = json.loads(token_file.read_text())
        assert saved["token"] == "refreshed_token"
...
def test_load_runtime_preferences(patch_data_paths):
    ...
    # Valid file
    patch_data_paths["settings"].write_text(json.dumps({
        "llm": {
            "provider": "ollama",
...
async def test_call_lm_studio_success():
    ...
    mock_resp.json.return_value = {
        "choices": [{
            "message": {"content": "LM Studio answer"}
        }]
    }
...
async def test_stream_answer_monitoring():
    ...
    with (
        patch("app.search.llm_client.capability_detector.detect_capabilities", AsyncMock(return_value=True)),
