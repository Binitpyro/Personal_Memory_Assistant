...
async def test_get_drive_fs_type():
    ...
    with patch("app.api.system.plat.system", return_value="Windows"):
        ...
        mock_kernel32.GetVolumeInformationW = lambda drive, vol, vol_len, serial, max_len, flags, fs, fs_len: setattr(fs, 'value', 'NTFS') or True
...
async def test_get_os_string():
    ...
    # Linux
    with patch("app.api.system.plat.system", return_value="Linux"):
...
async def test_compact_db_endpoints(client):
    ...
    with patch.dict(os.environ, {"UVICORN_WORKER_ID": "1"}):
        ...
        assert response.json()["message"] == "Compaction can only be triggered by the primary worker."
...
async def test_demo_seed_endpoint(client, tmp_path):
    ...
    # Folder exists
    with patch("app.api.system.os.path.isdir", return_value=True):
