...
from app.search.planner import QueryPlan, PlanMode
...
class MockDB:
    ...
    async def get_all_folder_profiles(self):
        return [{"folder_tag": "tag", "total_size_bytes": 1024 * 1024, "project_type": "python", "folder_path": "/p", "file_count": 5, "top_extensions": "py", "key_files": "a.py", "profile_text": "desc"}]
...
class MockLanceDB:
    ...
    async def search_summaries(self, vector, k, filter_dict=None):
        return {"metadatas": [[{"chunk_id": "1", "file_path": "a.py", "folder_tag": "tag", "_embedding": [0.1]*384}]]}
...
async def test_determine_metadata_insights():
    ...
    stats = {"total_files": 1, "total_size_mb": 1.0, "by_type": [{"type": ".py", "count": 1, "size_mb": 1.0}], "by_folder": [{"folder_tag": "tag", "count": 1, "size_mb": 1.0}]}
    profiles = [{"folder_tag": "tag", "total_size_bytes": 100, "project_type": "py", "folder_path": "/p", "file_count": 1, "top_extensions": "py", "key_files": "", "profile_text": ""}]
...
async def test_build_fast_answer():
    # FAST_METADATA mode
    plan_meta = QueryPlan(mode=PlanMode.FAST_METADATA, original_query="how many files", intents={"inventory": True})
    ...
    # FAST_PROJECT mode
    plan_proj = QueryPlan(mode=PlanMode.FAST_PROJECT, original_query="projects", intents={"project": True})
    profiles = [{"folder_tag": "tag", "total_size_bytes": 100, "project_type": "py", "folder_path": "/p", "file_count": 1, "top_extensions": "py", "key_files": "", "profile_text": ""}]
...
async def test_hybrid_retrieve_cache_and_lengths():
    ...
    res2 = await retrieval.hybrid_retrieve("hello nice doc in project fold tag", db, emb, lancedb, k=2, use_reranker=False)
    assert len(res2) >= 0

    # Query with long words (>8 words)
    res3 = await retrieval.hybrid_retrieve("hello nice doc in project fold tag with more long sentence here", db, emb, lancedb, k=2, use_reranker=False)
    assert len(res3) >= 0
